...
public class SpiderAI : MonoBehaviour
{
    ...
    private float _grassEnterReduction = 5f;
    private float _crouchReduction = 4f;
    
    public float screechRadius = 5f;
    ...
    public Image AwarenessFillImage;
    ...
    public void UpdateAwarenessMeter(float awareness)
    {
        if(gameObject.CompareTag("Hatchling"))
        {
            return;
        }
        AwarenessFillImage.fillAmount = awareness;
    }
    public void ChangeAwarenessMeterColor(Color color)
    {
        AwarenessFillImage.color = color;
    }
    private void Awake() {
    ...
    {
        if(PlayerTransform.gameObject.GetComponent<InteractionManager>().IsHiding)
        {
            Debug.Log("Spider cannot see hiding player");
            
            if(PlayerTransform.gameObject.GetComponent<PlayerProceduralAnimator>().IsCrouching())
            {
                DetectionRadius = _originalDetectionRadius-_crouchReduction-_grassEnterReduction;
            }else{
                DetectionRadius = _originalDetectionRadius-_grassEnterReduction;
            }
        }else if(PlayerTransform.gameObject.GetComponent<PlayerProceduralAnimator>().IsCrouching())
        {
            DetectionRadius = _originalDetectionRadius-_crouchReduction;
        }else{
    ...
    public IEnumerator IsThePlayerStillBeingChasedIfNotSwitchMusic()
    {

        float timer = 10f;

        // Have to stay atleast 10 seconds without being chased by a spider, before music changes
        while(timer > 0)
        {
            timer -= Time.deltaTime;
            if(m_currentState == ChasingState)
            {
                timer = 10f;
            }
            yield return null;
